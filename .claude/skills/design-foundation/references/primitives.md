...
**Web implementation:** `apps/web/src/app/globals.css` (partial token set). Prefer that file for runtime values; use this doc for the full scale and rules.
...
**Web (self-hosted):** `apps/web/src/app/fonts/` via `next/font/local` in `layout.tsx` → CSS variable `--font-euclid` / Tailwind `font-sans`. Format: **WOFF2**, `display: "swap"`.

| Weight | Value | File (shipped) |
|--------|-------|----------------|
| Regular | 400 | `EuclidCircularB-Regular.woff2` |
| Medium | 500 | `EuclidCircularB-Medium.woff2` |
| Semibold | 600 | `EuclidCircularB-Semibold.woff2` |
| Bold | 700 | `EuclidCircularB-Bold.woff2` |

Light (300) and italics are not shipped — do not use Light for body text. Always use `font-sans` / `var(--font-euclid)` rather than hardcoding the family name.
...
| Asset | Path | Usage |
|-------|------|-------|
| App icon (SVG) | `apps/web/src/app/icon.svg` → `/icon.svg` | Primary brand mark in UI / marketing |
| App icon (PNG) | `apps/web/public/icons/icon-192.png`, `icon-512.png` | PWA / install icons |
| Maskable | `apps/web/public/icons/icon-maskable-*.png` | Adaptive / maskable PWA icons |

**Brand rules:**
- Prefer `/icon.svg` as the primary DriveScore mark (includes wordmark)
...
| Type | Source | Usage |
|------|--------|-------|
| UI icons | Prefer `currentColor` SVGs / icon set used by the app | Navigation, actions, status |
| Brand | `/icon.svg` or `/icons/icon-*.png` | App identity |
